function [B, totSig, R, pCha, sigmaU] = frugFun5_uniformOddballs_wRange(data, Hazard, noise, drift, likeWeight, initGuess, inRun, dataRange)
...
for i = 1:length(data)
   ...
   % normalize=((normcdf(300, B(i), totSig(i)))- normcdf(0, B(i), totSig(i)));
    ...
    % 3 terms 1) oddball prob * change-point variance =
    % current variance
    % 2) non-oddball prob * non-oddball varuance (get one more
    % data point.
    % 3) difference in means^2 * p(X)p(Y)
    ss=pCha(i).*((sigmaE.^2)./(R(i)))...
        +pNoCha.*((sigmaE.^2)./(R(i)+1))...
        +pCha(i).*pNoCha.*((B(i)+yInt.*Delta)-B(i)).^2;
