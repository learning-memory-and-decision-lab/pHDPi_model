...
% Load outcomes, predictions, cps and test id
...
% Calculate LRs, find cps for each half 
...
% Calculate LRs, find cps for each window 
...
for i=1:length(myFiles)  
        ...
        cp = find(model_cp(i,switch_index-twin+1:switch_index)==1,k) + switch_index - twin;
        [val,~]=intersect(pre_lr_sel(i,:),cp);
        pre_switch_late(i,1:length(val)) = trial_LR(i,val);

        cp = find(model_cp(i,switch_index+1:switch_index+twin)==1,kk) + switch_index;
        [val,~]=intersect(post_lr_sel(i,:),cp);
        post_switch_early(i,1:length(val)) = trial_LR(i,val);

        cp = find(model_cp(i,end-twin+1:end)==1,kk) + switch_index*2 - twin;
        ...
        cp = find(model_cp(i,switch_index+1:end)==1)+switch_index;
