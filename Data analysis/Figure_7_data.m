...
myDir = '/Users/niloufarrazmi/Brown Dropbox/Niloufar Razmi/2024/Projects/HDP/Paper/Code/june_24_2025'; %gets directory
    myFiles = dir(fullfile(myDir,'*.mat')); %gets all wav files in struct
num_trials = 400;
num_sub = length(myFiles)/2;

task_1_gamma = nan(num_sub,num_trials);
task_1_alpha = nan(num_sub,num_trials);
task_1_eta = nan(num_sub,num_trials);

task_2_gamma = nan(num_sub,num_trials);
task_2_alpha = nan(num_sub,num_trials);
task_2_eta = nan(num_sub,num_trials);
 task_1_count  = 1;
 task_2_count  = 1;
...
for i = 1:length(myFiles)
    ...
    if contains(baseFileName,'_1_2')
          task_1_gamma(task_1_count,:) = output.mle_gamma;
          task_1_alpha(task_1_count,:) = output.mle_alpha;
          task_1_eta(task_1_count,:) = output.mle_eta;
          task_1_count  = task_1_count+1;

    else
          task_2_gamma(task_2_count,:) = output.mle_gamma;
          task_2_alpha(task_2_count,:) = output.mle_alpha;
          task_2_eta(task_2_count,:) = output.mle_eta;
