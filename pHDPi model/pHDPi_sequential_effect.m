...
function[output] = pHDPi_sequential_effect(taskData)

trialOutcome = taskData.outcome;
outcomeNoise=taskData.noise;
...
comb.driftVariance = [0.01,25, 225, 625, 1000];
comb.stdLogspace = [100, 225, 625, 1000];
allCombos=makeAllCombos(comb);
totalSamps = length(allCombos.alpha);
nCopy = 15; % Copy of nodes with same parameters
...
particle_prob = zeros(numTrials,numSamps);
...
for i = 1: numTrials
    ...
    particle_prob(i,:) = exp(nodes.logLike)/sum(exp(nodes.logLike));
    ...
    for n=1:length(nodes.logLike)
        ...
        if i==1
            ...
            output.nodes_ll(n,i) = nodePosteriorProb(nodes.currentState(n));
        end
        ...
        if clearTransitions
            if i == numTrials/2
                nodes.dist{n}           =initStruct;
                nodes.max_n_context(n) =max_context;
                %nodes.logLike(n)             =log(1);  % = posterior prob
                nodes.currentState(n)        =1;  %
                nodes.nextState(n)        =1;  %
                nodes.beta{n} = zeros(1,nodes.max_n_context(n));
                stick_l =  betarnd(1,nodes.gamma(n));
                nodes.beta{n}(1)= 1;
                nodes.beta{n}(2) = 0;
                nodes.mean(n,:) = zeros(1,numTrials);
                nodes.transitionCounts{n} = zeros(nodes.max_n_context(n),nodes.max_n_context(n));
...
output.particle_prob =particle_prob;
