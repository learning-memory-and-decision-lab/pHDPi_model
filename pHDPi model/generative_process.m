function output = generative_process(input)
...
elseif input.taskType ==4
   ...
   while~isfinite(outcome(i))|outcome(i)>screenWidth|outcome(i)<1;
    ...
    % History-dependent task (Prat-carrabin)
elseif input.taskType ==5
    
    run_length = zeros(numOutcomes,1);
    distMean(1)=round(rand(1).*screenWidth);

    for i=2:numOutcomes
        qt = 1/(1+exp(-1*(run_length(i-1,1)-10)));
        if rand<qt
            mean=round(rand(1).*screenWidth);
            cp(i)=1;
            run_length(i,1) = 0;
        else
            run_length(i,1) = run_length(i-1,1)+1;
        end
